...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^ax_") ; then
       AX_BUILD=$(echo -n $1 | sed -e 's/^ax_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $AX_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       AX_BUILD=
    fi
    export AX_BUILD
